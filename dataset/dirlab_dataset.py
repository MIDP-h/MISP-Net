...
class DIRLabDataset(Dataset):
    ...
    def __init__(self, data_root='./x/dirlab', mode='train',inshape=128):
        if mode=='train':
            data_idx = [1,2,4,5,6,7,8,9,10]
            # data_idx = [1]
        if mode=='val':
            data_idx = [3]
            # data_idx = [1]
        data_list = [ ("Case%dPack" % (i) , "case%d_T%02d" % (i,j)) for i in data_idx for j in range(0, 100,10)]
